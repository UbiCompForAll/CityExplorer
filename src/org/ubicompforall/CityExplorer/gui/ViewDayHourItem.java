...
public class ViewDayHourItem extends LinearLayout
{
	...
	public class InvisibleView extends View
	{
		...
		public InvisibleView(Context context)
		{
			super(context);
			setFocusable(false);
			LinearLayout.LayoutParams layParams = new LinearLayout.LayoutParams(android.view.ViewGroup.LayoutParams.FILL_PARENT, android.view.ViewGroup.LayoutParams.WRAP_CONTENT);
	...
	private static final float fTextSize = 35;
	...
	//colors
	private final static int iColorTime = 0xFF4556A6;//0xFFFF8800;
	private final static int iColorMark = 0xBB4556A6;//0xFFAA5500;
	private final static int iColorTimeFocused = 0xFFFFCC66;
	private final static int iColorTimeTouch = 0xFFFFFFFF;//222222;
	private final int iTouchSliderColor = 0xBB4556A6;//FF8800;
	private final int iTouchPatternColor = 0xFF4556A6;//995500;
	...
	//views
	private ArrayList<poiTextView> poiTextViews = new ArrayList<ViewDayHourItem.poiTextView>();
	...
	public ArrayList<poiTextView> getPoiTextViews() {
		return poiTextViews;
	}
	...
	protected void onDraw(Canvas canvas)
	{
        ...
        private void DrawTime(Canvas canvas, int iTimePosY, int iWidth, int iHeight, boolean bIsFocused)
        {
        	...
        	if(b24HourMode)
        	{
        		if(Calendar.getInstance().get(Calendar.HOUR_OF_DAY) == iHour)
        			correctHour = true;
        	}
        	else
        	{
        		if(Integer.toString(Calendar.getInstance().get(Calendar.HOUR)).equals(GetHourString()))
        		{
        			if(
        					(Calendar.getInstance().get(Calendar.AM_PM) == Calendar.AM && GetUSTimeMark().equals("am"))
        				||	(Calendar.getInstance().get(Calendar.AM_PM) == Calendar.PM && GetUSTimeMark().equals("pm"))	
        			   )//correct time mark
        			{
        				correctHour = true;
        			}
        ...
        public boolean onTouchEvent(MotionEvent event)
        {
        	...
        	{               
        		...
        		{                                               
        			...
        			invalidate();
        			startAlphaAnimIn(ViewDayHourItem.this);
        		}
        	}
        ...
        public class poiTextView extends TextView implements Comparable<poiTextView>
        {
        	private Paint mpt;
        	...
        	private boolean walkingEntry = false;
			...
			public void setAsWalkingEntry(boolean walkingEntry) {
				this.walkingEntry = walkingEntry;
			}
			
			public boolean isWalkingEntry()
			{
				return walkingEntry;
			}
			...
			public void setMinutes(int i) {
				if(i < 10)
					this.minutes = "0"+Integer.toString(i);
				else
					this.minutes = Integer.toString(i);
			...
			private int getSpaceWidthHour(Paint mpt)
			{
				mpt.setTypeface(tfMono);
				mpt.setTextSize(fTextSize);
				mpt.setFakeBoldText(true);
				return (int)mpt.measureText(":"+minutes+" ");
			}
			...
			public void draw(Canvas canvas) 
			{
	        	...
	        	super.setHeight((int)(-mpt.ascent() + mpt.descent()));
	        	super.setPadding(0, 0, 0, 0);
	        	
	        	int SpaceWidthHour = getSpaceWidthHour(mpt);
	        	float ascent = mpt.ascent();
	        	//draw hour
	        	/*final String sHour = GetHourString();
	        	final int iStrHourWidth = (int)mpt.measureText(sHour);          
	        	final int iTimePosX = iSpaceWidthHourArea - iStrHourWidth - iFrame - iSpaceWidthUSTimeMark - iSpace;*/
	        	...
	        	canvas.drawText(":"+minutes, 0, -mpt.ascent(), mpt);
	        	
	        	mpt.setFakeBoldText(false);
	        	mpt.setTextSize(fTextSize-3);
	        	
	        	if(walkingEntry)
	        	{
	        		mpt.setTextSize(fTextSize-10);
	        		canvas.drawText(text, SpaceWidthHour, -ascent, mpt);
	        	}
	        	else
	        	{
	        		canvas.drawText(poi.getLabel(), SpaceWidthHour, -ascent, mpt);
	        	}
			}

			public ViewDayHourItem getHourItem()
			{
				return ViewDayHourItem.this;
			}
			
			@Override
			public int compareTo(poiTextView another) {
				Integer inte1 = this.getMinutes();
				Integer inte2 = another.getMinutes();
